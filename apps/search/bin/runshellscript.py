    ...
    if not os.path.exists(script):
        ...
        try:
            ...
            if mswindows and use_cmd_exe:
                ...
                if sum(len(arg) for arg in cmd_args_ms) > 8155:
                    ...
                    cmd2run_ptr = 2
                    while (sum(len(arg) for arg in cmd_args_ms) > 8155) and cmd2run_ptr < 9:
                        cmd_args_ms[cmd2run_ptr] = "ARGUMENT_TOO_LONG"
                        cmd2run_ptr += 1

                #
                # Need to give Popen a string otherwise it will do its own escaping too
                cmd_args_ms_str = ""
                for i in range(len(cmd_args_ms)):
                    if cmd_args_ms[i].find("^") != -1 or cmd_args_ms[i] == '':
                        cmd_args_ms_str += " \"" + cmd_args_ms[i] + "\" "
                    else:
                        cmd_args_ms_str += cmd_args_ms[i] + " "
