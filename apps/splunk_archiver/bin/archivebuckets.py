...
import splunk.vix.vixutils as vixutils
import json
import splunk.vix.splunkio as splunkio
import time
import itertools

import splunk.vix.erp_launcher as erp_launcher
...
def hasValidLicense():
    si = en.getEntities('/server/info')
    addOns = si['server-info']['addOns']
    if addOns is None or 'hadoop' not in addOns:
        return False
    return True
...
def execute():
    try:
        if not hasValidLicense():
            return vixutils.generateErrorResults('Permission denied.  License does not allow archivebuckets command to run')
