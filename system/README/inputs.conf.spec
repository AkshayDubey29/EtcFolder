...
* Determines whether or not the Event Collector input should write its configuration to a deployment server repository.
* When set to 1 (enabled), the input writes its configuration to the directory that you specify with the 'repositoryLocation' attribute in serverclass.conf. You must copy the full contents of the splunk_httpinput app directory to this directory for the configuration to work.
* When enabled, the input writes its configuration to $SPLUNK_HOME/etc/apps by default.
